...
var focusTrap = __webpack_require__(/*! focus-trap */ "./node_modules/focus-trap/dist/focus-trap.esm.js");

window.__trappedElements = [];
var AlpineTrapMagicMethod = {
  start: function start() {
    Alpine.addMagicProperty('trap', this.trap.bind(null, null));
    Alpine.addMagicProperty('release', this.release.bind(null, null));
  },
  trap: function trap() {// let releasableElement = __trappedElements.filter(([trappedTarget, trap]) => trappedTarget.isEqualNode(target)).shift()
    //
    // if (releasableElement === undefined) {
    //     throw new Error('Can not release non-existing element')
    // }
    //
    // [element, trap] = releasableElement
    //
    // trap.deactivate()
    //
    // window.__trappedElements = __trappedElements.filter(trapped => __trappedElements.indexOf(trapped) !== __trappedElements.indexOf(releasableElement))
  },
  release: function release(el) {// const trap = focusTrap.createFocusTrap(target, options)
    // trap.activate()
    // __trappedElements.push([target, trap])
  }
};
...
window.deferLoadingAlpine = function (callback) {
  console.log('HELLO');
  AlpineTrapMagicMethod.start();
  alpine(callback);
};

var livewireShouldObserve = function livewireShouldObserve(el) {
  return !el.tagName.includes("-") || el.hasAttribute('wire:observe');
};
/**
